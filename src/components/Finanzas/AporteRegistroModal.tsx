...
interface AporteRegistroModalProps {
  isOpen: boolean;
  onClose: () => void;
  /** Recibe la fila creada (tras insert + select) para actualizar la lista al instante. */
...
const AporteRegistroModal: React.FC<AporteRegistroModalProps> = ({ isOpen, onClose, onSaved }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!row) {
        setFormError('El aporte se guardó pero no se pudo leer la fila. Pulsa Actualizar.');
        return;
      }
