    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="flex-1 min-w-[200px]">
          <h1 className="text-2xl font-bold text-gray-900">⏱️ Valor tiempo</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Equivalente a la hoja <strong>TIEMPO</strong> del Excel — operativo por vehículo (no es ingreso ni gasto).
          </p>
          <button
            type="button"
            onClick={() => navigate('/operaciones/control-global')}
            className="text-xs font-semibold text-indigo-600 hover:text-indigo-800 mt-2 underline underline-offset-2"
          >
            ← Volver a Control Global
          </button>
        </div>
      </div>

      <Card title={editingId ? 'Editar registro' : 'Nuevo registro'} subtitle="F. registro = cuándo cargaste; fecha = día del hecho">
      ...
      <Card title="Filtros y listado" subtitle={`${filtered.length} registro${filtered.length !== 1 ? 's' : ''}`}>
                ...
                {filtered.map((r) => (
                  <tr key={r.id} className="border-b border-gray-50 hover:bg-gray-50/80 align-top">
                    ...
                    <td className="py-2 px-3 text-right font-medium tabular-nums">
                      {r.valorTiempo != null ? r.valorTiempo : '—'}
                    </td>
